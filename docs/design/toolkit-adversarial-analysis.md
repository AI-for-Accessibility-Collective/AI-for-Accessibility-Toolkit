...
- **Compound/intersecting needs** where one adaptation's output is another's input (deaf+dyslexic → does the simplify pass run on the caption stream? low-vision+RTL+magnification → does enlargement break bidi?), and the **older-adult/temporary-overlap majority** who reject the disability frame entirely ("helpful to someone who will never tick a disability box").
