...
- **See it work first** (Node 24+, no build step): `cd toolkit && npm install && npm test`, then run `npm run demo:xr` (or, from the repo root, `node toolkit/hosts/xr-demo/demo.js`) — the whole loop (onboard → grant → XR render → insight back → consent) on in-memory ports.
...
- **Executable cross-surface proof**: `node toolkit/test/cross-surface-translation-test.js` (from the repo root) shows a web-side change rendering into XR and an XR-originated update rendering into mobile — copy its patterns for reads and writes.
- **Two integration styles**:
  - *JS runtime (WebXR, Electron, RN)*: embed the core — implement 4 small ports (`KVStore`, `Clock`, `Scheduler`, `Consent`; contracts in `toolkit/ports/index.js`) and call `createToolkit(...)`. Copy `toolkit/platforms/node/` as your template host.
  - *Any language / thin client*: call the hosted HTTP service — `POST /v1/librarian/{method}` with a bearer token (see [../server/CONTRACT.md](../server/CONTRACT.md); ask a maintainer for the team instance URL and a token). Native (C#/Swift) apps that only need profile transfer can implement the JSON protocol in `toolkit/protocol/` instead (schemas + fixtures; ArtInsight's Swift conformer is the worked example).
