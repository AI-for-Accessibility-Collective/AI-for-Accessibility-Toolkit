...
# The engine's public surface, in two groups.
#
# Group one is what the package is actually used through: every name
# `cli/cli.py` reaches as `_engine().<name>`, plus the four more that
# `cli/tests/` reaches. Computed from those two callers, not chosen by hand.
#
# Group two is everything else the single-module engine happened to expose at
# module level. These are internal. They are listed only so that any import
# that worked before the split still works, and nothing outside the package
# should depend on them.
#
# The list stays explicit rather than a star import so a reader can check the
...
__all__ = [
    ...
    # Internal. Exported only to keep the pre-split import surface working.
    "_CLI_DIR",
    "OUT",
    "_CLI_TOOLS_BUNDLE",
    "_READABILITY_PATH",
