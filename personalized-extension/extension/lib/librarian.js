...
// setGeminiCaller, same pattern as BrowserAgent.
...
Return ONLY valid JSON with:
{
  ...
  "newSkills": [ /* ONLY if the user's need CANNOT be fully met by the settings and built-in tools above, suggest custom skills to build. Each object has "name" (short) and "description" (1-2 sentences of what it would do). Leave as empty array [] if existing settings are sufficient. */ ]
    ...
    async logObservation(obs) {
      ...
      if (globalThis.AA_DEMO_MODE && obs.type === 'agent-task') {
        if (!origin) origin = 'youtube.com';
        if (!category) category = 'video';
      }
      const taskSucceeded = obs.data?.success
        || (globalThis.AA_DEMO_MODE && obs.type === 'agent-task');
      if (obs.type === 'agent-task' && taskSucceeded && origin && category) {
        if (globalThis.aaDemoTrace) aaDemoTrace('skill', 'reusable_q', 'common reusable task?');
    ...
    async _maybeProposeReusableAction(obs, origin, category) {
      ...
      if (exists && !globalThis.AA_DEMO_MODE) return;
      const profile = await getOrInitProfile();
      ...
      if (globalThis.aaDemoTrace) aaDemoTrace('skill', 'reusable_q', 'proposed: save as skill');
      await updateBadge();
    },
      ...
      if (response === 'accept') {
        ...
        if (prop.change?.op === 'profile-set') {
          ...
          if (globalThis.aaDemoTrace) {
            aaDemoTrace('skill', 'skillsdb', 'saved as skill.md');
            aaDemoTrace('skill', 'autoenable', 'skill stored');
            aaDemoTrace('skill', 'profiledb_skill', 'trigger registered');
            aaDemoTrace('personal', 'continual', 'continual update');
          }
...
- Return ONLY JSON: {"operations":[...], "proposals":[...]}`;
    ...
    async _draftProposals(drafts, { suppressions, profile, now }) {
      ...
      const demo = !!globalThis.AA_DEMO_MODE;
      const props = await DS().get('mine.proposals');
      ...
      const cap = demo ? Infinity : (profile.metaPreferences?.maxProposalsPerWeek ?? 3);
      for (const d of drafts) {
        if (!d.aspect || !d.change) continue;
        if (weekCount >= cap) break;
        const sup = demo ? null : suppressions.find(s => s.aspect === d.aspect && (s.mode === 'permanent' || s.until > now));
        if (sup) continue;
  ...
  // The SW may die before a setTimeout fires; the alarms are the guarantee,
  // the timeout is the fast path.
