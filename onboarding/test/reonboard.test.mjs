...
// First onboard: vision, explicitly blind. A typed id can no longer CREATE a
// profile (ids are capabilities), so the first onboard has to take the uid it
// is given back and re-onboard against that. Passing a typed id here would
// quietly create a generated profile and leave every assertion below reading
// an empty one, which passes for the wrong reason.
const retest = (await onboard({ supportAreas: ['vision'], freeText: '', visionKind: 'blind' })).uid;
...
const freetext = (await onboard({ supportAreas: ['reading'], freeText: 'I need quiet pages' })).uid;
...
await onboard({ uid: freetext, supportAreas: ['reading'], freeText: 'one sentence' });
await onboard({ uid: freetext, supportAreas: ['reading'], freeText: 'a different sentence' });
