...
// Librarian object, no librarian* message equivalent) — CONTRACT.md "41 routes
// total".
...
const ALL_ROUTES = [...EXTENSION_ALIAS_ROUTES, ...DIRECT_SURFACE_ROUTES];
assert.equal(ALL_ROUTES.length, 41, 'contract total must be 41 routes');
...
async function main() {
  ...
  try {
    ...
    // ---- meta: the full 36-route alias set -----------------------------------
    await test('GET /v1/meta lists all 41 routes (36 extension aliases + 5 direct-surface), all supported', async () => {
